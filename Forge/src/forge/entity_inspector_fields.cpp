...
namespace forge
{
                                     ...
                                     *value = checked;
                                     undo.Push(
                                         [&world, entity, typeId, offset, before]() {
                                             ...
                                             if (comp != nullptr)
                                                 *reinterpret_cast<bool*>(static_cast<std::byte*>(comp) + offset) =
                                                     before;
                                         },
                                                     ...
                                                     const FieldContext& ctx, QFormLayout* form)
    {
        ...
        if (field.m_nestedTypeId == queen::TypeIdOf<hive::math::Float3>())
        {
            ...
            {
                QColor initial = QColor::fromRgbF(static_cast<double>(floats[0]), static_cast<double>(floats[1]),
                ...
                QObject::connect(btn, &QPushButton::clicked, this,
                                 [this, btn, floats, snapshot, entity, typeId, offset, &undo, &world]() {
                                     QColor cur = QColor::fromRgbF(static_cast<double>(floats[0]),
            ...
            auto snapshot = std::make_shared<SnapshotState>();
            return BuildAxisRow(
                3, configs, [this, floats, snapshot, entity, typeId, offset, &undo, &world](int axis, float newVal) {
        ...
        if (field.m_nestedTypeId == queen::TypeIdOf<hive::math::Quat>())
        {
            ...
            auto snapshot = std::make_shared<SnapshotState>();
            return BuildAxisRow(3, configs,
                                [this, q, snapshot, entity, typeId, offset, &undo, &world](int axis, float newDeg) {
                                    float curEuler[3];
                                                   ...
                                                   const FieldContext& ctx)
    {
        ...
        for (size_t i = 0; i < field.m_enumInfo->m_entryCount; ++i)
        {
            ...
            combo->addItem(QString::fromUtf8(entry.m_name), QVariant{static_cast<qlonglong>(entry.m_value)});
        ...
        QObject::connect(
            ...
            [this, combo, fieldData, fieldSize = field.m_size, entity, typeId, offset, &undo, &world](int index) {
                if (index < 0)
                    return;
                int64_t newValue = combo->itemData(index).toLongLong();
                auto beforeData = std::make_shared<std::vector<std::byte>>(
                    static_cast<std::byte*>(fieldData), static_cast<std::byte*>(fieldData) + fieldSize);
                ...
                undo.Push(
                    [&world, entity, typeId, offset, fieldSize, beforeData]() {
                        void* comp = world.GetComponentRaw(entity, typeId);
                        if (comp != nullptr)
                            std::memcpy(static_cast<std::byte*>(comp) + offset, beforeData->data(), fieldSize);
                    },
                    [&world, entity, typeId, offset, fieldSize, afterData]() {
                        void* comp = world.GetComponentRaw(entity, typeId);
                        if (comp != nullptr)
                            std::memcpy(static_cast<std::byte*>(comp) + offset, afterData->data(), fieldSize);
                    });
                emit sceneModified();
            });
